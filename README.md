...
A polished, interactive tourist website for Rasnov, Romania, featuring an AR treasure hunt, real-time location data from Google Places API, and comprehensive visitor information.

## ✨ New: Google Places API Integration

This website now features **automatic, real-time data** powered by Google Places API:

- 🎯 **Real Location Data**: Tourist attractions, restaurants, and accommodations fetched from Google Places
- 📸 **Authentic Photos**: High-quality images from Google Places and Unsplash
- ⭐ **Live Ratings & Reviews**: Real visitor ratings and review counts
- 🕐 **Opening Hours**: Current status and weekly schedules
- 📍 **Accurate Coordinates**: Precise locations on the interactive map
- 🔄 **Auto-Updates**: Data refreshes weekly via GitHub Actions
- 💰 **100% Free**: Built-time fetching stays within Google's generous free tier

**[📖 Read the Setup Guide](SETUP_GUIDE.md)** to configure your own API keys and enable data fetching.
...
### Build-Time Fetching (What We Do) ✅
```
Google Places API → fetch-places-data.js → places-data.json → Static Site
     (Once/week)          (Node.js)            (Deployed)      (No API calls)
```

**Benefits**:
- 💰 FREE: ~400 API calls/month << 28,000 free tier limit
- ⚡ FAST: No API latency for visitors
- 🔒 SECURE: API keys never exposed to browser
- 📦 SIMPLE: Deploy anywhere as static site

### Runtime Fetching (What We DON'T Do) ❌
```
Every Page Load → Google Places API → Response → Render
   (Expensive)       (Costs $$$$)
```

**Problems**:
- 💸 EXPENSIVE: Could easily exceed free tier
- 🐌 SLOW: Every visitor waits for API response
- ⚠️ INSECURE: API keys exposed in browser
- 🔧 COMPLEX: Requires backend/proxies
...
├── .github/
│   └── workflows/
│       └── update-places-data.yml   # Auto-update workflow
├── build-scripts/
│   └── fetch-places-data.js         # Data fetching script
├── data/
│   ├── places-data.json             # Real data (generated)
│   └── sample-places-data.json      # Sample fallback data
├── js/
...
- Nearby Search: 3 calls × $32/1000 = $0.096
- Place Details: 60 calls × $17/1000 = $1.02
- Place Photos: 180 refs × $7/1000 = $1.26
- **Total**: ~$2.38/week = **$9.52/month**

**Remaining**: $190.48/month for other projects! 🎉
...
- ✅ ~~Integration with Google Maps/OpenStreetMap~~ (Done!)
- ✅ ~~Real location data with Google Places API~~ (Done!)
- ✅ ~~Automatic data updates via GitHub Actions~~ (Done!)
